...
class Reference:
	...
	def __str__(self):
		return "Ref number: %d; Authors: %s; Title: %s; Source: %s; Abstract: %s" % (self.ref_number, self.authors, self.title, self.source, self.abstract)
...
try:
	with open(input_file) as f:
		...
		for line in f:
			...
			if line.startswith("<"):
				ref_index += 1
				ref_number = int(line.strip("<").rstrip(">"))
				author_list =''
				title=''
				source=''
				ref_list[ref_index] = Reference()
				ref_list[ref_index].ref_number = ref_number
        ...
        # volume, issue, start page and finish page.
            ...
            # names as a list: 
				...
				ref_list[ref_index].authors = author_list
