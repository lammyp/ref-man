...
def split_EMBASE_source(so_field):
    """This function takes the "Source" field and splits it into standardised
    components, returning these as a dictionary."""

    (journal, numbers) = so_field.split(".", 1)
    journal = journal.strip()
    (numbers_main, month_year) = numbers.split(".", 1)
    (vol_iss_pag, year) = numbers_main.split(",")
    (volume, issue, pages) = vol_iss_pag.split("(")
    volume = volume.strip()
...
if 'medline' in str(input_file).lower():
    file_type = 'medline'
elif 'embase' in str(input_file).lower():
    file_type = 'embase'
elif 'central' in str(input_file).lower():
    file_type = 'central'
else:
