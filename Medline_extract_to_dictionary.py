...
def split_source(so_field):
    ...
    source_parts = {"journal":journal, "volume":volume, "issue":issue, "pages":pages, "year":year}
    return source_parts
...
c1 = con1.cursor()
try:
    c1.execute('''CREATE TABLE ml1(id INT NOT NULL, Authors TEXT NOT NULL, Year INT NOT NULL)''')
...
try:
    with open(input_file) as f:
        ...
        # Initialise a dictionary to eventually accept entries corresponding to 
        #records:
        ...
        for line in f:
            ...
            if line.startswith("Authors") == True:
                ...
                current_ref["Authors"] = author_list_clean
            ...
            # the code may change when I find a better solution:
                ...
                #print(current_ref, "\n")
                print(current_ref["Index"])
                print(current_ref["Authors"])
                print(current_ref["Journal"])
                print(current_ref["Year"])
                print(current_ref["Title"])
                ref_data = (current_ref["Index"], current_ref["Authors"], current_ref["Journal"], current_ref["Year"], current_ref["Title"])
                print(ref_data)
                short_data = (ref_count, str(current_ref["Authors"]), int(current_ref["Year"]),)
                print(short_data)
                print(type(short_data[0]))
                print(type(short_data[1]))
                print(type(short_data[2]))
